...
export default function DashboardLayout({
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Sidebar */}
      <aside className={`fixed top-0 left-0 bottom-0 w-64 glass border-r border-border z-40 transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 pt-16 lg:pt-0`}>
        <div className="flex flex-col h-full">
          ...
          <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
            ...
            <Link
              ...
              className={`flex items-center space-x-3 px-4 py-3 rounded-lg transition-colors ${pathname.startsWith('/dashboard/notifications') ? 'gradient-bg text-white' : 'hover:bg-surface-elevated'}`}
              onClick={() => setIsSidebarOpen(false)}
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
